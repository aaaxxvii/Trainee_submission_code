...
print("\nベースラインモデルの学習開始")
baseline_model.fit(X_train_base, y_train_base, batch_size=32, epochs=100, validation_split=0.1, verbose=0)
print("ベースラインモデルの学習完了")


# ベースラインモデルの予測と評価
predictions_scaled_base = baseline_model.predict(X_test_base, verbose=0)
...
print(f"方向性精度 (1日単位): {directional_accuracy_daily_base:.2f}%")
print(f"方向性精度 (1週間単位): {directional_accuracy_weekly_base:.2f}%")
...
# 最適化の実行
print("\nベイズ最適化を開始")
study.optimize(objective, n_trials=20, show_progress_bar=False, n_jobs=-1)
...
# テストデータで予測を実行
predictions_scaled = final_model.predict(X_test, verbose=0)
...
print(f"方向性精度 (1日単位): {directional_accuracy_daily:.2f}%")
print(f"方向性精度 (1週間単位): {directional_accuracy_weekly:.2f}%")
